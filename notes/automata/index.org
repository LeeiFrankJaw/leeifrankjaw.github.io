  ...
  \begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
    ...
    \path[->] (A) edge node {0} (B)
              ...
              (B) edge node {0} (E)
                  ...
                  edge [bend left] node [above right=.8cm and -3pt] {2} (D)
              (C) edge [bend left=15] node [below left=.4cm and -1pt] {0} (B)
                  edge node {1} (E)
                  edge [bend left=15] node [above right=.4cm and -1pt] {2} (D)
              (D) edge [bend left] node [below left=.8cm and -3pt] {0} (B)
                  edge [bend left=15] node [above left=.4cm and -1pt] {1} (C)
                  edge node [below right] {2} (E)
              (E) edge [loop above] node {0,1,2} (E);
  \end{tikzpicture}
#+END_SRC
...
Inductive Hypotheses:
 ...
 2. If $\hat\delta(A, w) = B$, then $w \in L$ and $w$ ends in $0$.
 3. If $\hat\delta(A, w) = C$, then $w \in L$ and $w$ ends in $1$.
 4. If $\hat\delta(A, w) = D$, then $w \in L$ and $w$ ends in $2$.
...
If $w$ is not accepted by this DFA, then $\hat\delta(A, w) = E$ and
$\abs{w} > 1$.  Let $w = xab$.
 * $\hat\delta(A, w) = \delta(\hat\delta(A, xa), b) = E$
 * $\hat\delta(A, xa) \in \{B,C,D\}$
   - If $\hat\delta(A, xa) = \delta(\hat\delta(A, x), a)= B$, then $b
     = a = 0$.  Thus, $w = x00$ and $w \notin L$
   - Do the same thing for $C$ and $D$.

Therefore, $w \notin L$.  Hence, every string in the language $L$ is
...
* TODO [[https://lagunita.stanford.edu/courses/course-v1:ComputerScience+Automata+SelfPaced/courseware/b9162be11c7b4c61a573a50e3aba5738/8c5876c1e4bb46af8fbf4a51ecfccbbc/][Regular Expressions and Properties of Regular Languages]]
...
** TODO Regular expressions
   ...
   :END:
** TODO Regular expressions in the real world
   DEADLINE: <2018-01-05 Fri>
   :PROPERTIES:
   :CUSTOM_ID: regular-expressions-in-the-real-world
   :END:
** TODO Regular-Expression Homework
   DEADLINE: <2018-01-06 Sat>
   :PROPERTIES:
   :CUSTOM_ID: regular-expression-homework
   :END:
** TODO Decision algorithms for regular languages
