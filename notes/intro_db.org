...
** IN-PROGRESS Lecture Videos
   :PROPERTIES:
   :CUSTOM_ID: views-and-authorization_mini-course-lectures
   :END:
...
#+BEGIN_SRC sql
  ...
  from Student, Apply
  where major = 'CS' and decision = 'Y'
        and Stduent.sID = Apply.sID and Apply.cName = 'Berkeley'
...
The following will sucessfully insert data into the underlying Apply
table.
...
#+BEGIN_SRC sql
  update Berk set major = 'physics' where major = 'psycology';
#+END_SRC
...
constrainst, Bob will be deleted from ~Apply~ table due to it.
...
*** IN-PROGRESS Materialized Views
