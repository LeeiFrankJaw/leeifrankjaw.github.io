...
* DONE [[https://lagunita.stanford.edu/courses/DB/Recursion/SelfPaced/courseware/ch-recursion/][Recursion in SQL]] mini-course
  CLOSED: [2018-01-14 Sun 16:02]
  :PROPERTIES:
  :CUSTOM_ID: recursion-in-sql-mini-course
  :END:
** DONE Lecture Videos
   CLOSED: [2018-01-13 Sat 23:09] DEADLINE: <2018-01-11 Thu>
*** DONE Basic Recursive WITH Statement---Introduction
    CLOSED: [2018-01-11 Thu 20:32]
    :PROPERTIES:
    :CUSTOM_ID: basic-recursive-with-statement-introduction
    :END:
...
SQL standard specifies ~recursive~ keyword goes with relation, but
implementations differ and make it as modifier for ~with~, and thus
any relations are allowed to be recursive if the ~recursive~ keyword
is given.
...
*** DONE Basic Recursive WITH Statement---Demo
    CLOSED: [2018-01-13 Sat 16:13]
    :PROPERTIES:
    :CUSTOM_ID: basic-recursive-with-statement-demo
    :END:
#+BEGIN_SRC sql
...
All the above queries work well given that there is *no loop* in our
Flight.
...
The above query works conceptually but PostgresSQL doesn't allow
recursive reference within a subquery.
...
infinite recursion.  However, if you replace ~*~ with ~min(total)~,
...
*** DONE Nonlinear and Mutual Recursion
    CLOSED: [2018-01-13 Sat 23:09]
    :PROPERTIES:
    :CUSTOM_ID: nonlinear-and-mutual-recursion
    :END:
Linear Recursion
...
This is a nonlinear recursion since there are two references to
relation Ancestor.
...
Mutual Recursion
 * recursive queries refer to recursive relations other than
   themselves.

They work in tandem.  SQL standard allows mutual recursion in limited
forms.
...
Hub: points to greater than or equal to 3 authority node
Authority: pointed to greater than or equal to 3 hub node
...
It's been said there is no implementation of mutual recursion in
common RDBMSs.  So the above query works only conceptually.
...
The above query has non-unique fixed points (nondeterministic) of the
recursion.  This is bad and thus SQL standard does't allow this
type of mutual recursion.  In fact, the crux of the problem is
negative dependence, which can also happen even without mutual
recursion.  Non-mutual recursion with negative dependence is not
allowed either.
...
Recursion with aggregation is disallowed in the SQL standard and isn't
...
** DONE Exercises
   CLOSED: [2018-01-14 Sun 16:02] DEADLINE: <2018-01-12 Fri>
*** DONE Recursion Quiz
    CLOSED: [2018-01-14 Sun 16:02]
    :PROPERTIES:
    :CUSTOM_ID: recursion-quiz
    :END:
