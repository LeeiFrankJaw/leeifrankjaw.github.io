...
\begin{definition*}
  设\(\Seq{a_n}\)是数列，\(A\)是常数．我们把\(A\)叫作数列\(\Seq{a_n}\)的极限，记作\(\displaystyle \lim_{n\to\infty} a_n = A\)，当且仅当：对于\kenten{任意的}\(ε > 0\)都\kenten{存在}正整数\(N\)使得\kenten{当}\(n > N\)\kenten{时}都有\(\abs*{\,a_n - A\,} < ε\)．
