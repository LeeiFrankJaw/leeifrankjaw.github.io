...
\item \pdfbookmark{6}{6} The interarrival times for cars passing a
  ...
  \item \currentpdfbookmark{d}{6.d} Consider the following two
    experiments:
    \begin{enumerate}
    \item \subpdfbookmark{i}{6.i} Pick a card at random from a group
    ...
    \item \currentpdfbookmark{ii}{6.ii} Come to the corner at a random
...
\item[G1{\footnotemark[2]}.] \footnotetext[2]{Required for 6.431;
    optional for 6.041} Consider a Poisson process with rate \(λ\),
  and let \(N(G_i)\) denote the number of arrivals of the process
  during an interval \(G_i = (t_i, t_i + c_i]\).  Suppose we have
  \(n\) such intervals, \(i = 1,2,··· , n\), mutually disjoint.
  Denote the union of these intervals by \(G\), and their total length
  by \(c = c_1 + c_2 + \dots + c_n\).  Given \(k_i \ge 0\) and with
